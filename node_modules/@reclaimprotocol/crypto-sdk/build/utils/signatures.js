"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.authoriseWalletForClaimCreation = exports.signatures = void 0;
var ethers_1 = require("ethers");
var sign_data_1 = require("./sign-data");
var utils_1 = require("ethers/lib/utils");
exports.signatures = {
    getPublicKey: function (privateKey) {
        var pub = (0, utils_1.computePublicKey)(privateKey, true);
        return ethers_1.utils.arrayify(pub);
    },
    getAddress: function (publicKey) {
        return (0, utils_1.computeAddress)(publicKey).toLowerCase();
    },
    sign: function (data, privateKey) {
        return __awaiter(this, void 0, void 0, function () {
            var wallet, signature;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        wallet = getEthWallet(privateKey);
                        return [4 /*yield*/, wallet.signMessage(data)];
                    case 1:
                        signature = _a.sent();
                        return [2 /*return*/, ethers_1.utils.arrayify(signature)];
                }
            });
        });
    },
    verify: function (data, signature, addressBytes) {
        return __awaiter(this, void 0, void 0, function () {
            var address, signerAddress;
            return __generator(this, function (_a) {
                address = typeof addressBytes === 'string'
                    ? addressBytes
                    : ethers_1.utils.hexlify(addressBytes);
                signerAddress = ethers_1.utils.verifyMessage(data, signature);
                return [2 /*return*/, signerAddress.toLowerCase() === address.toLowerCase()];
            });
        });
    }
};
function getEthWallet(privateKey) {
    if (!privateKey) {
        throw new Error('Private key missing');
    }
    return new ethers_1.Wallet(privateKey);
}
var DEFAULT_MINT_EXPIRY_M = 15;
/**
 * Authorise "requestor" to create a claim for "me"
 * using the given application data
 * @param me wallet who will own the credential
 * @param requestor wallet that will actually talk to the SC
 * @param data info about the credential to mint
 */
function authoriseWalletForClaimCreation(me, requestor, data) {
    return __awaiter(this, void 0, void 0, function () {
        var expiryMs, serialised, signature;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    expiryMs = Date.now() + (DEFAULT_MINT_EXPIRY_M * 60 * 1000);
                    serialised = (0, sign_data_1.createSignDataForClaim)(__assign(__assign({}, data), { owner: requestor.toLowerCase(), timestampS: Math.floor(expiryMs / 1000), epoch: 0 }));
                    return [4 /*yield*/, exports.signatures.sign(Buffer.from(serialised), me.privateKey)];
                case 1:
                    signature = _a.sent();
                    return [2 /*return*/, { signature: signature, expiryMs: expiryMs }];
            }
        });
    });
}
exports.authoriseWalletForClaimCreation = authoriseWalletForClaimCreation;
//# sourceMappingURL=signatures.js.map